...
def mp_keypoint_id_table(reverse=False):
    id2name = { 0: 'nose',
                1: 'left_eye_inner',
                2: 'left_eye',
                3: 'left_eye_outer',
...
def  mp_cocoid(id2name_coco , id2name_mp):
    name2id_coco = {v: k for k, v in id2name_coco.items()}
    name2id_mp = {v: k for k, v in id2name_mp.items()}
    mp2coco = {}
    for k, v in name2id_mp.items():
        if k in name2id_coco:
            mp2coco[v] = name2id_coco[k]
    return mp2coco  #num in 33 to num in 17

def getcocokeypoints(mp_keypoints , mp2coco):
    coco_keypoints = np.zeros((len(mp2coco) , mp_keypoints.shape[1]))
    for k, v in mp2coco.items():
        coco_keypoints[v, :] = mp_keypoints[k, :]
